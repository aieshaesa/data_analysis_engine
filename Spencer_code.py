...
class Airport:
    ...
    def print_analysis(self):
        ...
        number = self.most_frequent(self.chosen_column)
        print(MAKE_YELLOW, "The month with the most delays was", int_to_month[number], ". There were", self.chosen_column.count(number), "delays that month.", RESET)
        ...
        for idx,i in enumerate(delays):
            if i == '1' and idx != 0:
                month.append(self.columns_as_lists[0][idx])
                days.append(self.columns_as_lists[1][idx])
        List = list(zip(month, days))
        for x in range(12):
        ...
        for x in range(12):
            for y in range(7):
                for idx,i in enumerate(List2):
                    if List2[idx][0] == str(x+1) and List2[idx][1] == str(y+1):
                        List3.append(List2[idx])
        month.clear()
        days.clear()
        for idx,i in enumerate(List3):
            month.append(List3[idx][0])
            days.append(List3[idx][1])
            
        
        for idx,x in enumerate(month):
            for y in range(7):
                if x == month[idx-1] and days[idx] == str(y+1) and [days[idx-1] == str(y+1) or days[idx-2] == str(y+1) or days[idx-3] == str(y+1)]:
                    if days[idx-1] == str(y+1):
                        tmp = int(days[idx])
                        days[idx] = str(tmp + 7)
                    elif days[idx-2] == str(y+1):
                        tmp = int(days[idx])
                        days[idx] = str(tmp + 14)
                    elif days[idx-3] == str(y+1):
                        tmp = int(days[idx])
                        days[idx] = str(tmp + 21)
        List3 = list(zip(month, days))
        print(List3)
        '''
